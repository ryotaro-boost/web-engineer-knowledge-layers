...
- **AIに任せやすいか:** **一部任せられる** — 単純なクエリのインデックス案を出させるのは AI が得意。ただし「複合インデックスのカラム順」「部分インデックスの条件」「カバリングインデックスの設計」は実クエリ集と書き込み頻度を見て人間が判断すべき。AI は「念のため全カラムに INDEX」を出しがちなのでAIコードレビュー観点で検出する
