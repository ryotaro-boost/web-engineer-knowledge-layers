...
| 行数 | フルスキャン（O(n)） | B-Treeインデックス（O(log n)） |
|------|-------------------|---------------------------|
| 1,000 | 1,000行を走査 | 約10回の比較 |
| 100万 | 100万行を走査 | 約20回の比較 |
| 1億 | 1億行を走査 | 約27回の比較 |

1億行でも27回のディスクアクセスで目的の行に到達できる。[[ファイルシステムとIO]]で学んだ通り、ディスクI/Oは極めて遅い操作であるため、アクセス回数の削減はレスポンスタイムに直結する。
...
| **Hash** | 等値検索のみ（範囲検索不可） | `WHERE email = 'a@b.com'`（PostgreSQLのみ） |
...
| マイグレーションで `CREATE INDEX` を無条件に使用 | 本番環境で長時間テーブルロックが発生する | PostgreSQLでは `CREATE INDEX CONCURRENTLY`、MySQLでは `ALGORITHM=INPLACE` を使う |
...
- **公式ドキュメント**: [MySQL - Optimization and Indexes](https://dev.mysql.com/doc/refman/8.0/en/optimization-indexes.html) — MySQL固有のインデックス最適化
