...
### 「どこに境界を引くか」はフレームワークごとに違う (発展)

同じ SSR でも、**サーバーとクライアントの境界をどの粒度で引くか** はフレームワークの設計思想によって分かれる。Next.js App Router は **コンポーネント単位**（React Server Components）、Remix / React Router v7 は **ルート単位**（`loader`）、[[TanStack-Start]] は **関数単位**（Server Functions）で境界を引き、さらにルートごとに `ssr: false` / `'data-only'` を指定する **選択的 SSR** を提供する。「1つのアプリ内で複数戦略を混在させる」という本トピックの原則が、フレームワーク API のレベルでどう表現されるかの具体例として読むとよい。
