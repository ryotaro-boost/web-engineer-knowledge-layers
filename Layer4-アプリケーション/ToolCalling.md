...
### 判断そのものはモデルに任せない — 語彙と規則の外部化

自律的に作業を進めさせる用途では、「どのツールを呼ぶか」の手前にある**業務上の判断**（誰にエスカレーションするか、承認が要るか）をモデルに委ねたくなる。しかしそれをやると、判断根拠が事後の作文になり監査もテストもできなくなる。判断基準を語彙と規則としてコード側に外部化し、モデルには「自然言語 ↔ 語彙」の変換だけをさせる構成が現実的で、社内の暗黙知を扱う場合やローカルLLM前提の場合は特に効く。→ [[暗黙知のオントロジー化とローカルAI]]
