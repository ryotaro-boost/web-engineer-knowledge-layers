...
|---|---|---|
| プレースホルダを使った標準 CRUD（Prisma 通常 API、SQLAlchemy ORM、`pg.query` の `$1`） | 実装・レビュー両方 AI 委任。AIコードレビュー観点で文字列結合の検出 | 必要なクエリ性能の妥当性確認、N+1 の有無 |
...
| エラーメッセージの本番抑制 | 実装は AI、AIコードレビュー観点で詳細スタック露出を検出 | 「何を本番のエラー応答に含めるか」のポリシー決定 |
