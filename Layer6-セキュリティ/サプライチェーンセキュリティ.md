...
AI コーディングエージェント自体が新たな攻撃面となるリスク（パッケージ幻覚・プロンプトインジェクション・シークレット漏洩など）については [[生成AIコーディングエージェントのセキュリティリスク]] で詳しく扱う。加えて、Claude Code の Skills / プラグインのようにエージェントの能力を拡張する配布物は、npm パッケージと同じ「取得した瞬間に実行権が渡る」性質を持ち、新たな配布経路を生んでいる（[[Claude-Skillsエコシステム]]）。ここでは AI が生成するコードに頻出するサプライチェーン関連のアンチパターンを示す。
