...
A → B の依存があるなら、B → A の依存は許さない。循環依存は関心の分離が不完全なサイン。依存の方向を制御する手法として[[SOLID原則]]の依存性逆転の原則がある。

**4. 横断的関心事は専用の仕組みで扱う**

ログ、認証、エラーハンドリングなどの横断的関心事（Cross-Cutting Concerns）は、ミドルウェアやデコレータ、アスペクト指向の仕組みで分離する。全モジュールにコピペするのは分離ではない。
...
class UserService
{
    public function __construct(
        ...
        private WelcomeEmailJob $welcomeEmail,
    ) {}
    ...
    public function register(array $data): User
    {
        ...
        // 通知は非同期ジョブに分離（関心の分離）
        dispatch($this->welcomeEmail->for($user));
        return $user;
    }
}
