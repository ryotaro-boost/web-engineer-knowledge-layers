...
- **世界全体**: Google が公開している IPv6 採用率の統計（[google.com/intl/en/ipv6/statistics.html](https://www.google.com/intl/en/ipv6/statistics.html)）によれば、Google サービスへのアクセスのうち IPv6 経由は概ね 40〜45% 前後とされている（2024-2026 年時点）。長期トレンドとしては漸増を続けている
- **地域差が大きい**: 同統計によれば、米国・インド・フランス・ドイツ等は 50% を超える一方、地域によっては 10% を下回る国もある。「世界平均」だけで判断せず、自社サービスの主要ユーザー所在地で見るのが実務的
- **モバイルキャリア**: 米国 T-Mobile USA など IPv6 をデフォルトとしたキャリアでは 90% 超のトラフィックが IPv6 とされる。日本では NTT の NGN 網経由で IPv6 IPoE が広く使われている
- **クラウド**: AWS、GCP、Azure はすべて IPv6 をサポート。VPC のデュアルスタックや IPv6-only サブネットも構成可能
