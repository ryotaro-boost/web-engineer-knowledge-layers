...
RLS のポリシー条件は内部的に `WHERE` 句として追加される。`tenant_id` で頻繁にフィルタされるなら、[[インデックス]]が必要:
...
なお、RLS ポリシーは全クエリに暗黙の WHERE 句として追加されるため、サブクエリや関数呼び出しを含む複雑なポリシーではパフォーマンスに影響する。ポリシーの条件はできるだけ単純に保ち、参照するカラムには[[インデックス]]を設定すること。
