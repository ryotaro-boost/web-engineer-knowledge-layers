...
マルチプロセス ─→ マルチスレッド ─→ イベント駆動 ─→ サーバーレス
(Apache prefork)  (Apache worker)   (Nginx/Node.js)  (Lambda/Vercel)
                       │
                       ↓
                  ハイブリッド
                  (Puma, Go)
...
```
            ┌─ Worker Process 1 ─→ リクエストA を処理
Master ─────┼─ Worker Process 2 ─→ リクエストB を処理
            ├─ Worker Process 3 ─→ リクエストC を処理
            └─ Worker Process 4 ─→ (待機中)
            各プロセスは独立したメモリ空間を持つ
```
...
```
クライアント → Nginx (静的ファイル/リバースプロキシ)
                 │
                 ↓ FastCGI プロトコル
              PHP-FPM Master
                 │
                 ├─ Worker 1 → PHP スクリプト実行
                 ├─ Worker 2 → PHP スクリプト実行
                 └─ Worker 3 → PHP スクリプト実行
```
...
```
Tomcat
├─ Acceptor Thread → 接続を受け付け
├─ Thread Pool (maxThreads=200)
│   ├─ Thread 1 → リクエストA（Servlet 実行）
│   ├─ Thread 2 → リクエストB（DB問い合わせ中...ブロック）
│   ├─ Thread 3 → リクエストC（Servlet 実行）
│   └─ ... 最大200スレッド
```
...
```
従来の Platform Thread:
  OS Thread 1 → [リクエストA ... DB待ち(ブロック) ... 完了]
  OS Thread 2 → [リクエストB ... DB待ち(ブロック) ... 完了]
  → 200リクエスト = 200 OS スレッド必要

Virtual Thread:
  OS Thread 1 → [A実行][B実行][C実行][A再開][D実行][B再開]...
  OS Thread 2 → [E実行][F実行][E再開][G実行]...
  → 数千リクエストを数個の OS スレッドで処理
```
...
```
Master Process (設定の読み込み、Worker の管理)
├─ Worker Process 1 (1スレッド + イベントループ)
│   epoll/kqueue で数千の接続を同時に監視
│   ├─ コネクションA: データ到着 → 処理 → レスポンス送信
│   ├─ コネクションB: 待機中（CPUを消費しない）
│   ├─ コネクションC: upstream からレスポンス到着 → 転送
│   └─ ... 数千の接続を1スレッドで管理
├─ Worker Process 2 (同様)
└─ Worker Process 3 (同様)
```
...
```
Node.js プロセス (1スレッドのイベントループ)
│
├─ イベントループ (V8 エンジン)
│   JS コードの実行はすべてここ（シングルスレッド）
│
├─ libuv スレッドプール (デフォルト4スレッド)
│   ファイルI/O、DNS解決など一部のブロッキング操作を担当
│
└─ OS のノンブロッキング I/O
    ネットワーク I/O は OS に委譲し、完了通知を受ける
```
...
```
Puma Master
├─ Worker Process 1 (独立した GVL)
│   ├─ Thread 1 → リクエストA (DB問い合わせ中 → GVL解放)
│   ├─ Thread 2 → リクエストB (Ruby コード実行中 → GVL保持)
│   └─ ... 最大16スレッド
├─ Worker Process 2 (独立した GVL)
│   ├─ Thread 1 → リクエストC
│   └─ ... 最大16スレッド
└─ Worker Process 3
    └─ ...

4 Workers × 16 Threads = 最大64同時リクエスト
```
...
Go プロセス
├─ Go ランタイムスケジューラ
│   OS Thread (P) 1 ←→ Goroutine A, Goroutine D, Goroutine G ...
│   OS Thread (P) 2 ←→ Goroutine B, Goroutine E, Goroutine H ...
│   OS Thread (P) 3 ←→ Goroutine C, Goroutine F, Goroutine I ...
│   OS Thread (P) 4 ←→ ...
│   (GOMAXPROCS = CPUコア数 分の OS スレッド)
│
└─ 各リクエスト = 1 Goroutine（スタック: 初期2KB、必要に応じて伸長）
```
...
```
Tokio ランタイム
├─ Worker Thread 1 (CPUコアにバインド)
│   └─ タスクスケジューラ → Task A, Task D, Task G ...
├─ Worker Thread 2
│   └─ タスクスケジューラ → Task B, Task E, Task H ...
└─ Worker Thread 3
    └─ タスクスケジューラ → Task C, Task F, Task I ...

各リクエスト = 1 async Task（Goroutine より更に軽量）
```
...
リクエスト到着
    │
    ↓
┌─ コールドスタート（初回 or スケールアウト時）──────────┐
│  1. コンテナ生成（数百ms〜数秒）                      │
│  2. ランタイム初期化（Node.js/Python/Java等の起動）    │
│  3. ハンドラ外のコード実行（DB接続確立等）             │
└──────────────────────────────────────────────────────┘
    │
    ↓
┌─ ウォームスタート（コンテナ再利用）──────┐
...
ユーザー → Vercel Edge Network (CDN)
              │
              ├─ 静的アセット → CDN から直接配信（最速）
              │
              ├─ Edge Functions → CDN エッジで実行
              │   ランタイム: V8 Isolate（Chrome と同じエンジン）
              │   起動: <1ms（コールドスタートがほぼない）
              │   制限: Node.js API の一部が使えない、実行時間短い
              │
              └─ Serverless Functions → リージョンの Lambda で実行
                  ランタイム: Node.js / Python / Go / Ruby
                  起動: Lambda と同等（コールドスタートあり）
                  制限: Lambda と同様
```
...
├─ トラフィックが変動的 / インフラを管理したくない
│   → サーバーレス（Lambda, Vercel）
│
├─ I/O バウンド中心（API サーバー、リアルタイム通信）
│   ├─ チームが Go / Rust に習熟 → Go net/http / Actix Web
│   ├─ Node.js エコシステムを使いたい → Node.js + Nginx
│   └─ Java チーム → Spring Boot + Virtual Threads (Java 21+)
│
├─ CPU バウンドな処理が多い
│   → Go / Rust / Java（マルチコア活用が容易）
│
└─ 既存の PHP/Ruby アプリ
    ├─ PHP → Nginx + PHP-FPM（実績ある構成）
    └─ Ruby → Puma（ハイブリッドモデル）
```
