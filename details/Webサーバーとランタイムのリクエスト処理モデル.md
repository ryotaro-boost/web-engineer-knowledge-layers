...
```mermaid
graph LR
    Master["Master"] --> WP1["Worker Process 1 → リクエストA を処理"]
    Master --> WP2["Worker Process 2 → リクエストB を処理"]
    Master --> WP3["Worker Process 3 → リクエストC を処理"]
    Master --> WP4["Worker Process 4 → (待機中)"]
    Note["各プロセスは独立したメモリ空間を持つ"]
```
...
graph TD
    ...
    subgraph CP2["Child Process 2"]
        ...
        T2_2["Thread 2 → リクエストD"]
        T2_3["Thread 3 → リクエストE"]
    end
    Master --> CP1
    Master --> CP2
```
...
graph TD
    Tomcat["Tomcat"]
    Tomcat --> Acc["Acceptor Thread → 接続を受け付け"]
    subgraph Pool["Thread Pool (maxThreads=200)"]
        T1["Thread 1 → リクエストA（Servlet 実行）"]
        T2["Thread 2 → リクエストB（DB問い合わせ中...ブロック）"]
        T3["Thread 3 → リクエストC（Servlet 実行）"]
        TN["... 最大200スレッド"]
    end
    Tomcat --> Pool
```
...
```mermaid
graph LR
    subgraph PT["従来の Platform Thread"]
        direction LR
        OS1["OS Thread 1"] --> A1["リクエストA ... DB待ち(ブロック) ... 完了"]
        OS2["OS Thread 2"] --> B1["リクエストB ... DB待ち(ブロック) ... 完了"]
        PT_Note["→ 200リクエスト = 200 OS スレッド必要"]
    end
    subgraph VT["Virtual Thread"]
        direction LR
        VOS1["OS Thread 1"] --> VA["[A実行][B実行][C実行][A再開][D実行][B再開]..."]
        VOS2["OS Thread 2"] --> VB["[E実行][F実行][E再開][G実行]..."]
        VT_Note["→ 数千リクエストを数個の OS スレッドで処理"]
    end
```
...
graph TD
    ...
    subgraph W1["Worker Process 1 (1スレッド + イベントループ)<br>epoll/kqueue で数千の接続を同時に監視"]
        CA["コネクションA:<br>データ到着 → 処理 → レスポンス送信"]
        CB["コネクションB:<br>待機中（CPUを消費しない）"]
        CC["コネクションC:<br>upstream からレスポンス到着 → 転送"]
        CD["... 数千の接続を1スレッドで管理"]
    end
...
graph TD
    ...
    Node --> EL["イベントループ (V8 エンジン)<br>JS コードの実行はすべてここ（シングルスレッド）"]
    Node --> Libuv["libuv スレッドプール (デフォルト4スレッド)<br>ファイルI/O、DNS解決など<br>一部のブロッキング操作を担当"]
    Node --> OSIO["OS のノンブロッキング I/O<br>ネットワーク I/O は OS に委譲し、<br>完了通知を受ける"]
```
...
graph TD
    Master["Puma Master"]
    subgraph WP1["Worker Process 1 (独立した GVL)"]
        T1_1["Thread 1 → リクエストA<br>(DB問い合わせ中 → GVL解放)"]
        T1_2["Thread 2 → リクエストB<br>(Ruby コード実行中 → GVL保持)"]
        T1_3["... 最大16スレッド"]
    end
    subgraph WP2["Worker Process 2 (独立した GVL)"]
        T2_1["Thread 1 → リクエストC"]
        T2_3["... 最大16スレッド"]
    end
    subgraph WP3["Worker Process 3"]
        T3["..."]
    end
    Master --> WP1
    Master --> WP2
    Master --> WP3
    Note["4 Workers × 16 Threads = 最大64同時リクエスト"]
```
...
```mermaid
graph TD
    subgraph GoProc["Go プロセス"]
        subgraph Sched["Go ランタイムスケジューラ<br>(GOMAXPROCS = CPUコア数 分の OS スレッド)"]
            P1["OS Thread (P) 1"] ---|多重化| G1["Goroutine A, D, G ..."]
            P2["OS Thread (P) 2"] ---|多重化| G2["Goroutine B, E, H ..."]
            P3["OS Thread (P) 3"] ---|多重化| G3["Goroutine C, F, I ..."]
            P4["OS Thread (P) 4"] ---|多重化| G4["..."]
        end
        Note["各リクエスト = 1 Goroutine<br>(スタック: 初期2KB、必要に応じて伸長)"]
    end
```
...
graph TD
    subgraph Tokio["Tokio ランタイム"]
        subgraph WT1["Worker Thread 1 (CPUコアにバインド)"]
            S1["タスクスケジューラ"] --> TA["Task A"]
            S1 --> TD["Task D"]
            S1 --> TG["Task G ..."]
        end
        subgraph WT2["Worker Thread 2"]
            S2["タスクスケジューラ"] --> TB["Task B"]
            S2 --> TE["Task E"]
            S2 --> TH["Task H ..."]
        end
        subgraph WT3["Worker Thread 3"]
            S3["タスクスケジューラ"] --> TC["Task C"]
            S3 --> TF["Task F"]
            S3 --> TI["Task I ..."]
        end
    end
    Note["各リクエスト = 1 async Task<br>(Goroutine より更に軽量)"]
```
...
graph TD
    ...
    CDN --> Edge["Edge Functions<br>CDN エッジで実行"]
    CDN --> SLS["Serverless Functions<br>リージョンの Lambda で実行"]

    Edge --- E1["ランタイム: V8 Isolate<br>(Chrome と同じエンジン)"]
    Edge --- E2["起動: &lt;1ms<br>(コールドスタートがほぼない)"]
    Edge --- E3["制限: Node.js API の一部が<br>使えない、実行時間短い"]

    SLS --- S1["ランタイム: Node.js /<br>Python / Go / Ruby"]
    SLS --- S2["起動: Lambda と同等<br>(コールドスタートあり)"]
    SLS --- S3["制限: Lambda と同様"]
```
