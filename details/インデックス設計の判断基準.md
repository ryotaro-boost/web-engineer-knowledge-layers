...
値の分布が偏っている場合、少数派だけにインデックスを張ると効果的。なお、部分インデックスはPostgreSQL固有の機能であり、MySQLではネイティブサポートされていない。MySQLではgenerated column（生成列）にフラグを持たせてインデックスを作ることで類似の効果を得る。
...
`UPDATE` のたびにインデックス上の該当エントリが削除・再挿入される。更新頻度が高いカラムにインデックスを張ると、書き込みコストが増大する。
...
-- ❌ ORDER BY対象を先頭に置くと、等値条件のカラムが最左プレフィックスルールで活用できない
...
```sql
-- MySQL 5.7+ の sys スキーマで未使用インデックスを検出
...
### 2. ORM のクエリがインデックスを使えていない（Python）

```python
# SQLAlchemy でも同様の問題が起きる
from sqlalchemy import select
from sqlalchemy.orm import Session, selectinload
from models import User, Order

# ❌ N+1: relationship を lazy load するとループごとにクエリが飛ぶ
def get_users_orders_bad(session: Session):
    users = session.scalars(select(User)).all()
    for user in users:
        # user.orders にアクセスするたびに SELECT ... WHERE user_id = ? が発行される
        print(user.name, len(user.orders))

# ✅ selectinload で一括取得 → idx_orders_user_id が IN 句で効く
def get_users_orders_good(session: Session):
