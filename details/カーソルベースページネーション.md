...
app.get('/api/posts', async (req: Request, res: Response) => {
  ...
  if (cursor) {
    try {
      // URL に載せるため base64url（'+/=' を '-_' に置換、パディング無し）を使う。
      // 'base64' だと URL エンコードが必要になり二重エンコード事故の温床。
...
func listPosts(db *sql.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if cursorParam != "" {
			// URL に載せるため URLEncoding（base64url）を使う
...
class PostController extends Controller
{
    public function index(Request $request): JsonResponse
    {
        ...
        if ($cursor) {
            // URL に載せる cursor は base64url（+ / = を - _ '' に置換）
        ...
        $nextCursor = $hasMore
            ? rtrim(strtr(base64_encode(json_encode(['id' => $data->last()->id])), '+/', '-_'), '=')
            : null;
...
def encode_cursor(data: dict[str, Any]) -> str:
    # URL に載せるため urlsafe_b64encode（'+/' を '-_' に置換）を使う。
    # パディング '=' は URL で問題になりうるため除去
    return base64.urlsafe_b64encode(json.dumps(data).encode()).rstrip(b"=").decode()


def decode_cursor(cursor: str) -> dict[str, Any]:
    try:
        # パディング除去済みの場合は復元してから decode する
