...
| 観点 | レイヤード | ヘキサゴナル（Ports & Adapters） | [[オニオンアーキテクチャ\|オニオン]] | クリーン |
|------|----------|--------------------------------|--------------------------------|---------|
| 提唱者 | — （伝統的） | Alistair Cockburn（1994 年に着想、2005 年に文書化） | Jeffrey Palermo（2008） | Robert C. Martin（2012） |
| 依存方向 | 上→下（UI→Domain→DB） | 内→外を禁止（Ports で境界定義） | 内→外を禁止（4層の同心円） | 内→外を禁止（4層の同心円） |
| DB 層の位置 | 最下層（Domain が DB に依存しがち） | 外側の Adapter | 最外層の Infrastructure | 最外層の Frameworks & Drivers |
| 最内層 | — | Domain（Ports は Domain 側に定義） | Domain Model（Entity・Value Object） | Entities |
| Use Case の扱い | Service 層に混在 | 明示されない | Application Services 層に**集約** | **独立した層**として明示 |
| 核心概念 | 層の分離 | Port（境界）と Adapter（差し替え可能な実装） | DDD と結合したドメインモデル中心主義 | 依存性の規則 + Entities/Use Cases の明確な分離 |
| 実用上の違い | DB 変更時に Domain 層まで影響しやすい | Port 定義が設計の中心 | 1サービスに複数ユースケースが集まる | 1ユースケース1クラスで分割 |

本質的にはどれも「ビジネスロジックを技術的詳細から分離する」という同じ目標を持つ。系譜としては **ヘキサゴナル → [[オニオンアーキテクチャ|オニオン]] → クリーン** の順に発展しており、後発になるほど「内側に何を置くか」がより明示的になっている。クリーンアーキテクチャの特徴は、Use Cases（アプリケーション固有のルール）と Entities（企業レベルのルール）を明確に分離した点にある。
...
- [[オニオンアーキテクチャ]] — 同じ系譜の先行パターン（2008）。Use Case を独立層化せず、ドメインモデルを最内層に置く
...
- *Hexagonal Architecture* — Alistair Cockburn（Ports & Adapters の原典、1994 年に着想し 2005 年に文書化。クリーンアーキテクチャの先行概念）
