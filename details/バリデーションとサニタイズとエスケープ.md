...
def process_comment(raw_input: dict) -> dict:
    ...
    # ② サニタイズ — 許可リストベースのHTMLサニタイズ（nh3はRust製で高速）
...
ReactやVueでHTMLをそのまま描画するAPIは、サニタイズ済みのHTMLを表示するために存在する。サニタイズされていないユーザー入力をこれらに渡すとXSSが発生する。使用する場合は必ずDOMPurifyやnh3等でサニタイズしてから渡す。
...
| 正規表現による自作HTMLサニタイザー | HTMLの構文解析は正規表現では不可能。難読化された攻撃を見逃す | DOMPurify、bluemonday、nh3等の実績あるライブラリを使用 |
...
- nh3 — Python向けHTMLサニタイザー（Rust製、bleachの後継推奨）
