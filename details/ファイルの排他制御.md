...
```
Nginx ワーカー 1 ──write()──→ ┌──────────────┐
Nginx ワーカー 2 ──write()──→ │ access.log   │ ← データが混ざる！
Nginx ワーカー 3 ──write()──→ └──────────────┘
```
...
- **`O_APPEND` での追記** — POSIX では `O_APPEND` フラグ付きの `write()` はアトミックな追記が保証される（`PIPE_BUF` サイズ以内、通常 4096 バイト）
...
```
プロセスA: lock() → write() → unlock()   ← ルールを守る → 安全
プロセスB: lock() → write() → unlock()   ← ルールを守る → 安全
プロセスC:          write()               ← ロック無視 → データ破損の原因
```
...
lock_data = struct.pack('hhllhh',
    fcntl.F_WRLCK,  # ロック種別: 書き込みロック
    0,               # whence: SEEK_SET
    100,             # start: オフセット 100 から
    100,             # len: 100 バイト分
    0, 0             # pid, type（カーネルが設定）
)
fcntl.fcntl(f.fileno(), fcntl.F_SETLKW, lock_data)  # ブロッキング取得

# ... バイト 100〜199 を読み書き ...

# ロック解放
unlock_data = struct.pack('hhllhh',
    fcntl.F_UNLCK, 0, 100, 100, 0, 0
)
fcntl.fcntl(f.fileno(), fcntl.F_SETLK, unlock_data)
f.close()
```
