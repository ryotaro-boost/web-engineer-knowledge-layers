...
graph TD
    A["処理失敗"] --> B{"代替手段はあるか?"}
    B -->|"キャッシュにある"| C["✅ 正当なフォールバック<br/>キャッシュから返す"]
    B -->|"別サービスで取得可能"| D["✅ 正当なフォールバック<br/>代替サービスに切り替え"]
    B -->|"デフォルト値に意味がある"| E["✅ 正当なフォールバック<br/>デフォルト値を返す"]
    B -->|"何もない"| F["❌ エラーを伝播させる<br/>空配列やnullをでっち上げない"]
```
...
flowchart TB
    ...
    D -->|"失敗"| D1["⚠️ 「在庫確認中」と表示し<br/>購入ボタンを無効化"]
